...
public class Tester {
    class TreePrint {
        ...
        private <T extends Comparable<?>> int maxLevel(WAVLTree.WAVLNode root) {
            ...
            return Math.max(maxLevel(root.getLeftChild()), maxLevel(root.getRightChild())) + 1;
        }
    ...
    public void createLeafList() {
		 ...
		 * leafs.add(node); }
		 */
        shtut(leafs, generatedTree.getRoot());
    }

    public void shtut(ArrayList<WAVLTree.WAVLNode> shtut, WAVLTree.WAVLNode node) {
        if (node == null) {
            return;
        }
        if (node.getLeftChild() == null && node.getRightChild() == null) {
            ...
            shtut(shtut, node.getRightChild());
        }
    }
    ...
    public boolean RanksTest(WAVLTree.WAVLNode node) {
        while (node != null) {
            int leftChildsRank = node.getLeftChild() != null ? node.getLeftChild().getRank() : -1;
            int rightChildsRank = node.getRightChild() != null ? node.getRightChild().getRank() : -1;
